...
export default function HomeApp() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const filteredSpots = parkingData.filter(spot => spot.statusId === 1 || spot.statusId === 4);
      
        const availableSpotsCount = filteredSpots.length;
        setAvailableSpots(availableSpotsCount);
        console.log("Los parking que estan disponibles son", availableSpots)
      } catch (error) {
  ...
  useEffect(() => {
    if (availableSpots !== null) {
      console.log("La cantidad de espacios disponibles son:", availableSpots);
      if (availableSpots > 0) {
        setNoAvailableSpots(false); 
        console.log(noAvailableSpots);
      } else {
    ...
    if (reservationID !== null){
      console.log("EL id de reservación es:", reservationID);
    }
  }, [availableSpots, userId, reservationID]);
  ...
  useEffect(() => {
    const getReservation = async () => {
      try {
        ...
        console.log("Reservación:", data[0]);
        if (data.length > 0) {
          setNoAvailableSpots(false);
          setHasReservation(true);
          setReservationID(data[0].id);
          console.log("Tiene una reservacion!");
        }
      } catch (error) {
        console.error(
  ...
  const handleCancelReservation = async (reservationID) => {
    ...
    try {
      ...
      console.error(
        `Hubo un error al cancelar la reservación ${error}`
      )
    }
  }
    ...
    <View className="flex flex-col items-center justify-between h-screen bg-primaryColor">
      ...
      {noAvailableSpots ? (
        <View className=" bg-secondaryColor w-full h-[22%] flex justify-center items-center rounded-t-3xl space-y-2">
          <Text className="text-white text-md px-6 text-center">
            No hay lugares de estacionamiento disponibles.
          </Text>
          <Text className="text-white text-md px-6 text-center">
        ...
        <View className=" bg-secondaryColor w-full h-[22%] flex justify-center items-center rounded-t-3xl space-y-2">
          <Text className="text-white text-md px-6 text-center">
            Sigue las instrucciones para encontrar tu lugar.
          </Text>
          <TouchableOpacity
            onPress={() => handleCancelReservation(reservationID)}
            className="border border-focusColor bg-primaryColor rounded"
          >
        ...
        <View className=" bg-secondaryColor w-full h-[22%] flex justify-center items-center rounded-t-3xl space-y-2">
          ...
          <Text className="text-white text-xs px-6 text-center">
            Se te asignará un lugar según la demanda de estacionamiento.
          </Text>
        </View>
      )}
    </View>
